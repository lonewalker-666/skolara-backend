...
import { sendOtp, verifyOtp, assertLoginWindow, assertSignupWindow } from '../services/otp';
...
app.post('/auth/otp/send', {
  schema: {
    body: { type: 'object', required: ['mobile'], properties: { mobile: { type: 'string', pattern: '^[0-9]{10}$' } } }
  },
  config: {
    rateLimit: {
      max: 1,
...
app.post('/auth/otp/verify', {
  ...
  try {
    ...
    return reply.send({ verificationId: rec.id, verified: true, expiresAt: rec.expires_at });
  } catch (e: any) {
  ...
  app.post('/auth/login', {
    ...
    try {
      const { verificationId, mobile } = req.body as { verificationId: string; mobile: string };
      ...
      if (!user) {
        // create a minimal user; full profile can be completed later
        user = await prisma.users.create({
          data: {
            mobile,
            email: `u_${mobile}@placeholder.local`,
            first_name: 'User',
            last_name: 'Skolara',
      ...
      } else if (!user.mobile_verified) {
        await prisma.users.update({ where: { id: user.id }, data: { mobile_verified: true } });
      }

      const accessToken = JwtService.signAccess({ sub: user.id, mobile: user.mobile, email: user.email });
      const refreshToken = JwtService.signRefresh({ sub: user.id, mobile: user.mobile, email: user.email });

      return reply.send({ accessToken, refreshToken, user: { id: user.id, mobile: user.mobile, email: user.email } });
    } catch (e: any) {
  ...
  app.post('/auth/signup', {
    schema: {
      body: {
        type: 'object',
        required: ['verificationId', 'first_name', 'last_name', 'email', 'course_type_id'],
        properties: {
          verificationId: { type: 'string', format: 'uuid' },
    ...
    try {
      const { verificationId, first_name, last_name, email, course_type_id } =
        req.body as { verificationId: string; first_name: string; last_name: string; email: string; course_type_id: number };

      const rec = await assertSignupWindow(prisma, verificationId);

      // ensure user exists or create one, now with full info
      let user = await prisma.users.findUnique({ where: { mobile: rec.mobile } });
      ...
      if (!user) {
        ...
        // update/enrich profile (idempotent)
      ...
      return reply.send({ accessToken, refreshToken, user: { id: user.id, email: user.email, mobile: user.mobile } });
    } catch (e: any) {
      const msg = e?.message === 'OTP_WINDOW_EXPIRED' ? 'OTP_WINDOW_EXPIRED' : 'UNAUTHORIZED';
  ...
  app.post('/auth/token', {
    ...
    try {
      ...
      const user = await prisma.users.findUnique({ where: { id: Number(payload.sub) } });
